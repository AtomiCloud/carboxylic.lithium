...
public readonly struct Result<TSucc>
{
    ...
    public static implicit operator Exception?(Result<TSucc> result)
    {
        return result._isSuccess ? default : result._exception;
    }
    ...
    public static implicit operator TSucc?(Result<TSucc> result)
    {
        return result._isSuccess ? result._value : default;
    }
    ...
    public static implicit operator Result<TSucc>(TSucc val)
    {
        return new Result<TSucc>(val);
    }
    ...
    public static implicit operator Result<TSucc>(Exception err)
    {
        return new Result<TSucc>(err);
    }
    ...
    {
        if (obj is not Result<TSucc> other)
            return false;

        return Match(
            value => value is not null && other.Match(succ => value.Equals(succ), _ => false),
            exn => other.Match(_ => false, exn.Equals)
        );
    }
    ...
    {
        return Match(value => value is null ? 0 : value.GetHashCode(), exn => exn.GetHashCode());
    }
    ...
    public TSucc Get()
    {
        return Match(x => x, err => throw err);
    }
    ...
    {
        return Match(s => s, _ => defaultImpl);
    }
    ...
    {
        return Match(_ => defaultImpl, e => e);
    }
    ...
    /// <typeparam name="TResult">The type of the result of the computation (unused)</typeparam>
    /// <returns>Either the Success, or a Failure</returns>
    public Result<TSucc> Do<TResult>(
        DoType type,
...
public static class Result
{
    ...
    /// <returns></returns>
    public static Func<TInput, Result<TSucc>> ToResultFunc<TInput, TSucc>(
    ...
    public static Func<TInput, Result<Unit>> ToResultFunc<TInput>(
