...
public static class AsyncResultLinqExtensions
{
    ...
    public static async Task<Result<TReturn>> Select<TSucc, TReturn>(
    ...
    /// <typeparam name="TReturn">Return type of function</typeparam>
    /// <returns>The Result of the computation</returns>
    public static async Task<Result<TReturn>> SelectMany<TSucc, TMed, TReturn>(
        this Task<Result<TSucc>> result,
        Func<TSucc, Task<Result<TMed>>> function,
        Func<TSucc, TMed, Task<TReturn>> returner
    )
        where TSucc : notnull
        where TReturn : notnull
        where TMed : notnull
    {
        return await result.ThenAwait(x =>
            function(x).ThenAwait(y => returner(x, y), Errors.MapNone)
        );
    }
    ...
    public static async Task<Result<TReturn>> SelectMany<TSucc, TMed, TReturn>(
        this Task<Result<TSucc>> result,
        Func<TSucc, Task<Result<TMed>>> function,
        Func<TSucc, TMed, TReturn> returner
    )
        where TSucc : notnull
        where TReturn : notnull
        where TMed : notnull
    ...
    public static async Task<Result<TReturn>> SelectMany<TSucc, TMed, TReturn>(
        this Task<Result<TSucc>> result,
        Func<TSucc, Result<TMed>> function,
        Func<TSucc, TMed, Task<TReturn>> returner
    )
        where TSucc : notnull
        where TReturn : notnull
    ...
    {
        return await result.ThenAwait(x =>
            function(x).ToAsyncResult().ThenAwait(y => returner(x, y), Errors.MapNone)
        );
    }
